...
export function FAQ() {
    ...
    return (
      <Stack gap="1" className="faq-container">
        <Text fontWeight="medium" className="faq-title">FAQ (คำถามที่พบบ่อย)</Text>
        <Accordion.Root value={value} onValueChange={(e) => setValue(e.value)} className="faq-list">
          {items.map((item, index) => (
            <Accordion.Item key={index} value={item.value} className="faq-item">
              <Accordion.ItemTrigger>
                <Span flex="1" className="arrow">{item.title}</Span>
                <Accordion.ItemIndicator />
              </Accordion.ItemTrigger>
              <Accordion.ItemContent>
                <Accordion.ItemBody className="faq-answer">{item.text}</Accordion.ItemBody>
              </Accordion.ItemContent>
            </Accordion.Item>
          ))}
        </Accordion.Root>
...
const items = [
    {
        ...
        text: "A: At XYZ Company, we pride ourselves on innovation and cutting-edge technology. Our products are crafted with a perfect blend of creativity and export functionality, setting them apart from the competition. We constantly strive to push the boundaries of what's possible to deliver a truly exceptional user experience."
    },
    {
        value: "item2",
